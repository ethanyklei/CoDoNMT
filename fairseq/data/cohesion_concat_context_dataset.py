...
def collate(
    ...
    def merge_link(key, samples, padded_tokens):
        old_result = [s[key] for s in samples]
        new_result = []
        assert len(old_result) == padded_tokens.size(0)
        for sample_idx in range(len(old_result)):
            new_result.append({})
            if len(old_result[sample_idx]) == 0:
                continue
            pad_length = padded_tokens[sample_idx].eq(pad_idx).sum().item()
            for word_idx in old_result[sample_idx]:
                orig_word_idx = np.array(old_result[sample_idx][word_idx], dtype=np.int32)
                new_result[-1].update({
                    int(word_idx) + pad_length: orig_word_idx + pad_length
                })
        assert len(new_result) == padded_tokens.size(0)
        return new_result
    ...
    #############################################################
    cohesion_links = None
    if samples[0]['cohesion_link'] is not None:
        cohesion_links = merge_link("cohesion_link", samples, src_tokens)
    ##############################################################
    ...
    new_cohesion_links = None
    if cohesion_links is not None:
        new_cohesion_links = []
        for new_idx in sort_order:
            new_cohesion_links.append(cohesion_links[new_idx])
        del cohesion_links
    ...
    if new_cohesion_links is not None:
        batch['net_input']['cohesion_links'] = new_cohesion_links

    return batch


class CohesionConcatContextDataset(FairseqDataset):
    ...
    def __init__(
        ...
        self.epoch = 0
        self.cohesion_links = cohesion_links
        ###########################################
    ...
    def __getitem__(self, index):
        ...
        cohesion_link = {}
        if self.cohesion_links is not None and str(index) in self.cohesion_links:
            cohesion_link = self.cohesion_links[str(index)]
    ...
    def cohesion_mask(self, item, dictionary, index, cohesion_link):
        link_words = cohesion_link.keys()

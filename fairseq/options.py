...
# NOTE: build datastore for Knn-based NMT 
# based on the validation args
def get_create_datastore_parser(default_task=None):
    parser = get_parser("Datastore", default_task)
    add_dataset_args(parser, train=True)
    add_distributed_training_args(parser, default_world_size=1)
    ############################
...
def get_knn_generation_parser(interactive=False, default_task="translation"):
    parser = get_parser("KNNGeneration", default_task)
    add_dataset_args(parser, gen=True)
    add_distributed_training_args(parser, default_world_size=1)
    add_model_args(parser)
    add_generation_args(parser)
...
def add_datastore_args(parser):
    ...
    group.add_argument("--dstore-size", metavar="N", default=1, type=int,
                       help="datastore size")
    group.add_argument("--dstore-dir", default=None, type=str, help="save dir for datastore")

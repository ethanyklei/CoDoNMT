...
import torch
from fairseq import checkpoint_utils, distributed_utils, models, optim, tasks, utils
...
class Trainer(object):
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                if hasattr(self.task, "load_pretrained_params"):
                    # missing_keys, unexpected_keys = self.get_model().load_state_dict(
                    #     state["model"], strict=False, 
                    # )
                    self.task.load_pretrained_params(self.get_model(), state['model'])
                else:
                    #########################################
                    # NOTE: We allow the model to be partially loaded with parameters from a pre-trained model.
                    # report the missing keys and unexpected keys
                    missing_keys, unexpected_keys = self.get_model().load_state_dict(
                        state["model"], strict=False, args=self.args
                    )
                    logger.warning(f'Load checkpoint(missing_keys): {missing_keys}')
                    logger.warning(f'Load checkpoint(unexpected_keys): {unexpected_keys}')
            ...
            try:
                with maybe_no_sync():
                    ...
                    if "predicted" in logging_output \
                        and hasattr(self.task.dataset("train"), "predicted_memory") \
                        and self.task.dataset("train").predicted_memory is not None:

                        predicted = logging_output["predicted"]
                        ids = sample['id'].cpu().numpy()

                        for id, pred in zip(ids, predicted):
                            self.task.dataset("train").predicted_memory[id][: len(pred)] = pred
                        del predicted

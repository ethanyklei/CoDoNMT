...
def main(args, override_args=None):
    ...
    if args.dstore_fp16:
        ...
        dstore_keys = np.memmap(os.path.join(args.dstore_dir, "keys.npy"), dtype=np.float16, mode='w+', 
                            shape=(args.dstore_size, args.decoder_embed_dim))
        dstore_vals = np.memmap(os.path.join(args.dstore_dir, "vals.npy"), dtype=np.int32, mode="w+", 
                            shape=(args.dstore_size, 1))
    else:
        logger.info("saving fp32")
        dstore_keys = np.memmap(os.path.join(args.dstore_dir, "keys.npy"), dtype=np.float32, mode='w+', 
                            shape=(args.dstore_size, args.decoder_embed_dim))
        dstore_vals = np.memmap(os.path.join(args.dstore_dir, "vals.npy"), dtype=np.int32, mode="w+", 
                            shape=(args.dstore_size, 1))
    ...
    for subset in args.valid_subset.split(","):
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(progress):
                ...
                if args.dstore_fp16:
                    dstore_keys[dstore_idx: dstore_idx + reduce_size] = features.detach().cpu().numpy().astype(np.float16)
                    dstore_vals[dstore_idx: dstore_idx + reduce_size] = target.unsqueeze(-1).cpu().numpy().astype(np.int32)
                else:
                    dstore_keys[dstore_idx: dstore_idx + reduce_size] = features.detach().cpu().numpy().astype(np.float32)
                    dstore_vals[dstore_idx: dstore_idx + reduce_size] = target.unsqueeze(-1).cpu().numpy().astype(np.int32)
            ...
            logger.info(f"{dstore_keys.shape}")
